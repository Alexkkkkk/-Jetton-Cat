...
const AI_AGENT = {
    ...
    // Отправка общей команды управления
    sendNeuralCommand: async (client: TonClient, master: Address, cmd: { minMint: bigint, entropyAdj: number, freeze: boolean, enableArbitrage: boolean }) => {
        ...
        // ID метода 0x4e455552 (NeuralCommand)
        const body = beginCell()
            .storeUint(0x4e455552, 32) 
            .storeCoins(cmd.minMint)
            .storeInt(cmd.entropyAdj, 32)
            .storeBit(cmd.freeze)
            .storeBit(cmd.enableArbitrage) // Добавлен флаг арбитража
            .endCell();
    ...
    sendArbitrageCommand: async (client: TonClient, master: Address, amount: bigint) => {
        ...
        const body = beginCell()
            .storeUint(0x42555942, 32) // ID метода Buyback
            .storeCoins(amount)
            .endCell();
...
async function run() {
    ...
    try {
        ...
        // Включаем арбитраж, если система перегрета и есть избыток резервов
